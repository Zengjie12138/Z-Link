...
{
    MPU_Write_Byte(MPU_MOTION_DET_REG, 0x22);           //运动阈值
...
//  Zero_Motion_Interrupt();                        //静止中断
    ...
    MPU_Write_Byte(MPU_INTBP_CFG_REG, 0X20);
...
/* 
体感按键函数：Mpu_Key()
返回值：UP DOWN LEFT RIGHT 
*/

uint16_t MPU_Sample = 0; 
uint8_t Mpu_Key(void)
{
	short aacx, aacy, aacz = 0;
	char UD_Flag = 0,LR_Flag = 0;
	
	if(MPU_Sample == 0)//不是所有时间都采集数据
	{
	...
	if(LR_Flag!=0&&UD_Flag!=0)//无效操作(晃太厉害了)
	{
		return 0;
	}
	else if(LR_Flag==0&&UD_Flag==0)
	{
		return 0;
	}
	else 
	{
		MPU_Sample = 1;                    //返回一次有效动作后，隔段时间再采集数据
		return LR_Flag|UD_Flag;
	}		
